...
import { WeekContentSkeleton } from "@/components/skeletons";
...
export default async function WeekPage({
  ...
  return (
    <>
      <ManagerTop
        active="settimana"
        right={<Suspense fallback={null}><RecentBadge /></Suspense>}
      />
      <Container maxWidth="lg" sx={{ py: 2.5 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 3, flexWrap: "wrap" }}>
          ...
          <GenerateDraftButton monday={monday} />
        </Box>

        <Suspense fallback={<WeekContentSkeleton />}>
          <WeekContent monday={monday} />
        </Suspense>
      </Container>
    </>
  );
...
/** Badge notifiche non lette: clic → elenco modifiche. Sparisce una volta lette. */
async function RecentBadge() {
  const store = await cookies();
  const seenRaw = store.get(NOTIF_SEEN_COOKIE)?.value;
  const seen = seenRaw ? new Date(seenRaw) : new Date(0);
  const count = await prisma.auditLog.count({ where: { entity: "availability", at: { gt: seen } } });
  if (count === 0) return null;
  return (
    <Link href="/manager/notifiche" style={{ textDecoration: "none" }}>
      <Chip clickable label={`🔔 ${count} aggiornamenti`} size="small" color="warning" />
    </Link>
  );
}

/** Contenuto della settimana: griglia a fasce, ore/avvisi, poster. */
async function WeekContent({ monday }: { monday: string }) {
  const days = weekDates(monday);
  const monthKey = monthKeyOf(monday);

  const [secretaries, weekShifts, monthShifts, impianti] = await Promise.all([
